Shader "ClothDynamics/URP_Lit_Cloth_2019"
{
    Properties
    {
        ...
        SubShader
    {
       ...
       Tags{"RenderType" = "Opaque" "RenderPipeline" = "UniversalPipeline" "IgnoreProjector" = "True"}
        ...
        void MyClothFunction_float(uint vertexId, out float3 vertex, out float3 normal, out float4 tangent)
        {
            vertex = positionsBuffer[vertexId];
            normal = normalsBuffer[vertexId];
            tangent = float4(cross(normal, float3(0, 1, 0)), 1);
        }
        ...
        myVaryings vert(myAttributes input)
        {
            #if defined(SHADER_API_D3D11) || defined(SHADER_API_GLCORE) || defined(SHADER_API_GLES3) || defined(SHADER_API_METAL) || defined(SHADER_API_VULKAN) || defined(SHADER_API_PS4) || defined(SHADER_API_XBOXONE)
                #ifdef USE_BUFFERS
                MyClothFunction_float(input.vertexID, input.positionOS.xyz, input.normalOS.xyz, input.tangentOS);
                #endif
            #endif
...
#if defined(SHADER_API_D3D11) || defined(SHADER_API_GLCORE) || defined(SHADER_API_GLES3) || defined(SHADER_API_METAL) || defined(SHADER_API_VULKAN) || defined(SHADER_API_PS4) || defined(SHADER_API_XBOXONE)
#ifdef USE_BUFFERS
                    MyClothFunction_float(v.vertexID, v.vertex.xyz, v.normal.xyz);
#endif
#endif
                    VertexOutput o;
